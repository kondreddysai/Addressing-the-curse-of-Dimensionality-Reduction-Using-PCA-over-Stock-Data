...
//		saveResults(results, args[0]);
	...
	private static double[][] parseData(String filename) throws IOException{
		...
		System.out.println("NO OF OBJECTS:"+dims[0]);
		System.out.println("NO OF ATTRIBUTES:"+dims[1]);
		double[][] data = new double[Integer.parseInt(dims[0])][Integer.parseInt(dims[1])];
		...
		for(int i = 0; i< data.length; i++) {
			...
			for(int j = 0; j < data[0].length; j++)  {
				data[i][j] = Double.parseDouble(vals[j]);
			}
	...
	private static void saveResults(double[][] results, String filename) {
		...
		for(int i = 0; i < results[0].length; i++) {
			for(int j = 0; j < results.length; j++) {
				try {
					out.write("" +  (Double.parseDouble(String.format("%.4g%n", results[j][i]))) +"");
		...
		try {
                out.close();
				System.out.println("New file:"+newFilename+ " Has a copy of these principal components");
            } catch (IOException e) {
                e.printStackTrace();
            }
	}
