...
class Matrix {
	...
	static EigenSet eigenDecomposition(double[][] input) {
		...
		for(int i = 0; i < Q.length; i++) {
			Q[i][i] = 1; //Q starts as an identity matrix     //IDENTITY MATRIX-Q- CREATED
		}
		boolean done = false;
	...
	static double[][] gramSchmidt(double[][] input)
	{
		...
		double[][] out = new double[input.length][input[0].length];   //create a matrix 3x3  with all zeros elements
		
		    
		for(int outPos = 0; outPos < out.length; outPos++)    //REPEAT FOR 3 ROWS
		{
			
			double[] v = input[outPos];    //TAKE ONE ROW AT A TIME   
					
			for(int j = outPos-1; j >= 0; j--)
			{
				
				double[] sub = proj(v, out[j]);     //PASS ONE ROW AND  -1, 0 ROWS (ONE ROW AT A TIME)
